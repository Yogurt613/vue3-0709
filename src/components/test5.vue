...
<template>
    <div id="app" class="container mt-4">
        ...
        <div class="addList">
            <div class="col-auto p-0">
                <div class="input-group mb-2">
                    ...
                    <input type="text" class="form-control event" placeholder="輸入待辦事項..." v-model="addTodo"
                        @keyup.enter="newTodo">
                    <button type="button" class="btn btn-outline-primary" @click="newTodo">新增</button>
                </div>
            </div>
        </div>
        <!-- 篩選選項 -->
        <div class="filter-options mb-3">
            ...
            <button class="btn btn-outline-primary" :class="{ active: filter === 'doing' }"
                @click="setFilter('doing')">進行中</button>
            <button class="btn btn-outline-primary" :class="{ active: filter === 'done' }"
                @click="setFilter('done')">已完成</button>
        </div>
        <!-- 事件列表 -->
        <ul class="listTo">
...
export default {
    ...
    methods: {
        newTodo() {
            ...
            this.todos.push({ title: this.addTodo, completed: false });
            this.addTodo = '';
        },
        removeTodo(index) {
